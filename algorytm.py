...
class Algoritm:
    def __init__(self, problem: ReverseKinProblem, iteration_per_time=2):
    ...
    def get_default_animation_info(self):
        ...
        animation_info['title'] = f"ES, popsize={self.algoritm_info['population_size']}"
    ...
    def _create_initial_population(self, population_size, sigma):
        ...
        self.current_population_objective_values, self.X, self.Y = self.problem.evaluate_population(self.current_population_solutions)
        self._check_collision_current_population()
    ...
    def _check_collision_current_population(self, sigma = 1):
        ...
        self.current_population_objective_values -= self.current_population_colliding_idx*0.5
        ## ver2: increase sigmas of incorrect solutions by start_sigma*0.5
        add_sigma_shape = self.current_population_sigmas[self.current_population_colliding_idx, :].shape
        self.current_population_sigmas[self.current_population_colliding_idx, :] = sigma * np.ones(add_sigma_shape)
    ...
    def _select_parent_indices(self, population_size, number_of_offspring, number_of_parents):
        self.fitness_values = self.current_population_objective_values - self.current_population_objective_values.min()
    ...
    def _evaluate_children(self):
        ...
        self.children_population_objective_values, self.childX, self.childY = self.problem.evaluate_population(self.children_population_solutions)
        ...
        self.children_population_objective_values -= self.children_population_colliding_idx*0.5
    ...
    def es(self, number_of_iterations, population_size, number_of_offspring, number_of_parents, sigma, tau, tau_0):
        ...
        while(self.termination_condition(number_of_iterations)):
            ...
            if np.mod(self.iteration, number_of_iterations//10) == 0:
